...
const historyGroups = [
  {
    ...
    items: [
      ...
      "How to get rich from youtube as an influe?",
      "Help me with web development tasks fro",
    ],
  },
  {
    title: "7 дней",
...
<template>
  ...
  >
    ...
    <div class="relative my-5 flex min-h-0 flex-1 flex-col">
      <div
        class="chat-history scrollbar-thin flex-1 overflow-y-auto space-y-5 px-1"
      >
        <div v-for="group in historyGroups" :key="group.title">
          <p
            v-if="!isCollapsed"
            class="mb-3 text-sm"
            :class="theme === 'dark' ? 'text-slate-500' : 'text-gray-400'"
          ...
          >
            <li v-for="item in group.items" :key="item">
              <a
                href="#"
                class="flex items-center transition hover:text-accent"
                :class="
                  isCollapsed
                    ? 'justify-center rounded-xl py-2 hover:bg-white/70 dark:hover:bg-white/10'
                    : ''
                "
                :title="item"
              >
    ...
    <div class="chat-footer shrink-0 space-y-4">
      ...
      >
        ...
        >
          <img
            class="profile-img size-10 shrink-0 rounded-full object-cover"
            :src="user.avatar"
            alt=""
          />
