 ...
 * Карточка предложения действия из chat-layout.html.
 */
...
function diffLineClass(line) {
  if (isAddition(line)) return 'text-green-300'
  if (isDeletion(line)) return 'text-red-300 line-through'
...
const cardClass = computed(() => {
  const map = {
    pending: 'bg-slate-900 border border-amber-500/30',
    approved: 'bg-slate-900 border border-green-500/20 opacity-70',
    rejected: 'bg-slate-900 border border-red-500/20 opacity-70',
    completed: 'bg-slate-900 border border-green-500/20 opacity-70'
  }
...
<template>
  <div class="flex items-start gap-3 p-4 rounded-2xl shadow-lg animate-fade-in" :class="cardClass">
    <!-- Status icon -->
    ...
    <div class="flex-1 min-w-0">
      ...
      <p class="text-sm text-slate-200 mb-3">
        <strong class="text-slate-300">Действие:</strong> {{ action.title }}
      </p>

      <p v-if="action.description" class="text-sm text-slate-400 mb-3">{{ action.description }}</p>

      <div v-if="action.diff && action.diff.length" class="bg-slate-950/60 rounded-xl p-3 mb-3 font-mono text-xs space-y-1 max-h-48 overflow-y-auto">
