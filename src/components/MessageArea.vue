...
/**
 * MessageArea.vue
 * Область сообщений из chat-layout.html (slate-950 dark theme).
 */
...
<template>
  <div class="flex-1 overflow-y-auto px-4 py-6 space-y-4 scrollbar-thin" ref="container">
    ...
    <!-- Connecting -->
    <div v-if="!isConnected && !error" class="flex flex-col items-center justify-center h-full gap-3 text-slate-500">
    ...
    <div v-if="messages.length === 0 && isConnected && !streamingContent && !clientMode" class="flex flex-col items-center justify-center h-full text-center px-6">
      <div class="w-14 h-14 rounded-2xl bg-blue-600/10 flex items-center justify-center mb-4">
      ...
      <p class="text-sm text-slate-500">Выберите сайт и начните диалог</p>
    </div>
    ...
    <div v-if="messages.length === 0 && isConnected && !streamingContent && clientMode" class="flex flex-col items-center justify-center h-full text-center px-6">
      ...
      <p class="text-sm text-slate-500">{{ startHint }}</p>
    </div>
    ...
    <div v-if="streamingContent" class="flex justify-start animate-fade-in">
      <div class="max-w-[75%] bg-slate-800 text-slate-100 rounded-2xl rounded-bl-md px-4 py-2.5 text-sm leading-relaxed whitespace-pre-wrap">
        {{ streamingContent }}
      </div>
    </div>
