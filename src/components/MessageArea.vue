...
<template>
  <div class="flex-1 overflow-y-auto px-4 py-6 space-y-4 scrollbar-thin" ref="container">
    <!-- Error -->
    <div v-if="error && !isConnected" class="px-4 py-2.5 bg-red-50 text-red-500 rounded-xl text-sm">
    ...
    <div v-if="messages.length === 0 && isConnected && !streamingContent && !clientMode" class="flex flex-col items-center justify-center h-full text-center px-6">
      ...
      <h2 class="text-lg font-semibold text-gray-800 mb-1">AI Pilot</h2>
      <p class="text-sm text-gray-500">Выберите сайт и начните диалог</p>
    </div>
    ...
    <div v-if="messages.length === 0 && isConnected && !streamingContent && clientMode" class="flex flex-col items-center justify-center h-full text-center px-6">
      ...
      <h2 class="text-lg font-semibold text-gray-800 mb-1">{{ startTitle }}</h2>
      <p class="text-sm text-gray-500">{{ startHint }}</p>
    </div>
    ...
    <div v-if="streamingContent" class="flex justify-start animate-fade-in">
      <div class="max-w-[75%] bg-gray-100 text-gray-900 rounded-2xl rounded-bl-md px-4 py-2.5 text-sm leading-relaxed whitespace-pre-wrap shadow-sm">
        {{ streamingContent }}
      </div>
    </div>
