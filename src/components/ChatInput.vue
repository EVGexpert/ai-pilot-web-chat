...
/**
 * ChatInput.vue
 * Компонент ввода из chat-layout.html.
 */
...
<template>
  <footer class="shrink-0 px-4 py-3 border-t border-slate-800 bg-slate-950/80 backdrop-blur-sm">
    <div
      class="flex items-end gap-2 bg-slate-900 border border-slate-700/60 rounded-2xl px-4 py-3 transition-colors max-w-3xl mx-auto"
      :class="{
        'border-blue-500/40': messageInput.trim() && isConnected && !isLoading,
    ...
    >
      ...
      <button
        ...
        disabled
        class="shrink-0 w-9 h-9 flex items-center justify-center rounded-xl bg-slate-800 text-slate-600 cursor-not-allowed transition-colors"
      >
