...
<template>
  <aside class="w-72 shrink-0 bg-slate-950 border-r border-slate-800 flex flex-col overflow-hidden h-screen">
      ...
      <div class="flex items-center justify-between">
        <div class="flex items-center gap-3">
          ...
          <div>
            ...
            <p class="text-xs text-slate-500">Управление сайтами</p>
          </div>
        </div>
        <div class="flex items-center gap-1">
          <ThemeToggle />
          <button
            class="md:hidden p-1.5 rounded-lg hover:bg-slate-800 text-slate-500 hover:text-slate-300 transition-colors"
    ...
    <div class="px-4 pt-4 pb-2">
      <div class="relative">
        ...
        <input
          type="text"
          placeholder="Поиск..."
          class="w-full bg-slate-900 border border-slate-800 text-sm text-slate-100 placeholder-slate-500 rounded-xl pl-10 pr-4 py-2 outline-none focus:border-blue-500/40 transition-colors"
        />
      </div>
    </div>
    ...
    <div class="flex-1 overflow-y-auto px-4 py-3">
      <h2 class="text-[11px] font-semibold text-slate-500 uppercase tracking-wider mb-2">Мои сайты</h2>
      ...
      <template v-if="sitesStore.currentSiteConversations.length > 0">
        <h2 class="text-[11px] font-semibold text-slate-500 uppercase tracking-wider mb-2">Недавние диалоги</h2>
        <div class="space-y-1">
          <div
            v-for="conv in sitesStore.currentSiteConversations"
            :key="conv.id"
            class="px-3 py-1.5 rounded-xl hover:bg-slate-800/50 transition-colors cursor-pointer"
          >
    ...
    <div class="px-4 py-4 border-t border-slate-800 mt-auto">
      <div class="flex items-center justify-between">
        ...
        <button
          class="shrink-0 p-2 rounded-lg hover:bg-slate-800 text-slate-500 hover:text-slate-300 transition-colors"
          @click="handleLogout"
          title="Выйти"
        >
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
